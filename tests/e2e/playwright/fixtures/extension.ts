...
/**
 * Persistent user-data-dir for the happy-path specs. Logging in once via
 * `pnpm e2e:login` writes the demo-account tokens into chrome.storage.local
 * inside this profile; subsequent test runs reuse the dir and skip the auth
 * flow entirely. The dir is gitignored.
 *
 * Override with BRIE_E2E_USER_DATA_DIR to pin a different path (e.g., CI
 * caches it across jobs), or BRIE_E2E_EPHEMERAL=1 to fall back to a fresh
 * temp dir per run (useful when reproducing a "logged-out" scenario).
 */
const PERSISTENT_USER_DATA_DIR = resolve(__dirname, '../../.userdata/chrome-profile');
...
const launchExtensionContext = async (): Promise<LaunchResult> => {
  const ephemeral = process.env.BRIE_E2E_EPHEMERAL === '1';
  const userDataDir = ephemeral
    ? mkdtempSync(join(tmpdir(), 'brie-pw-happy-'))
    : (process.env.BRIE_E2E_USER_DATA_DIR ?? PERSISTENT_USER_DATA_DIR);

  // Chrome 130+ silently ignores --load-extension when developer mode is
  // OFF in the user profile. Seeding Default/Preferences with
  // developer_mode=true lets the extension actually install on first launch;
  // skip the write if Preferences already exists so we don't clobber a
  // logged-in session's other prefs (cookies, sync state, etc).
  const defaultProfileDir = join(userDataDir, 'Default');
  const prefsFile = join(defaultProfileDir, 'Preferences');
  if (!existsSync(prefsFile)) {
    mkdirSync(defaultProfileDir, { recursive: true });
    writeFileSync(prefsFile, JSON.stringify({ extensions: { ui: { developer_mode: true } } }));
  }
  ...
  return { context, userDataDir, ephemeral };
};

const teardownExtensionContext = async (result: LaunchResult | undefined) => {
  await result?.context.close();
  // Only delete the dir for ephemeral runs. The persistent dir is the whole
  // point — it carries the demo-account login forward between test runs.
  if (result?.ephemeral && result.userDataDir) {
 ...
 * it stays dormant until something pings the extension. We probe-navigate
 * to an http(s) page so the content_scripts host_permissions matcher fires
 * and brings the worker up.
 */
